...
def get_Nstep_returns(rewards, state_vals, gamma):
...
# NOTE: Needs discussion or investigation -@hyeongkyunkim at 12/12/2022, 4:16:59 AM
# TODO def train
# # https://arxiv.org/pdf/1506.02438.pdf
#
# https://github.com/nikhilbarhate99/Actor-Critic-PyTorch
# https://github.com/nikhilbarhate99/Actor-Critic-PyTorch/blob/master/train.py
...
class Actor(nn.Module):
    ...
    def _distribution(self, obs):
        ...
        distribution = Categorical(F.softmax(output, dim=0))
        return distribution
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # size = self.ptr - self.path_start_idx + 1
        # episode_weights = get_weights(size, self.lam)
        # episode_weights = [np.array(x) for x in episode_weights]

        # episode_returns = get_Nstep_returns(rews, vals, self.gamma)
        ...
        # G = ((G - G.mean())/G.std()).numpy()
        # self.tdres_buf[self.path_start_idx:self.ptr] = G[:-1]
...
class Agent:
    ...
    def step(self, state):
        ...
        v = self.critic.forward(state).detach().numpy().item()

        act = pi.sample()
        _, log_prob = self.actor.forward(state, act)
